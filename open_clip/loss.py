...
class CLIPSLoss(ClipLoss):
    ...
    def forward(self, image_features, text_features, logits, labels, logit_scale, output_dict=False):
        if self.clip_loss_weight:
            half = text_features.shape[0] // 2
            text_features_1 = text_features[:half]
            text_features_2 = text_features[half:]

            clip_loss_1 = super().forward(image_features, text_features_1, logit_scale)
            clip_loss_2 = super().forward(image_features, text_features_2, logit_scale)

            clip_loss = 0.5 * (clip_loss_1 + clip_loss_2)
            clip_loss = self.clip_loss_weight * clip_loss
        else:
            clip_loss = torch.tensor(0, device=logits.device)
